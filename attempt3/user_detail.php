...
{
    $id=$_POST['id'];
    $rentals_sql="SELECT user_id,rental_start_date FROM rentals WHERE car_id=$id";
    $rentals_result=mysqli_query($conn,$rentals_sql);
    $num_users = mysqli_num_rows($rentals_result);
}
?>
...
<!-- main code  -->
    ...
    <section class="all-section">
        ...
        <div class="row justify-content flex-wrap align-items-left">
            <h2 class="section-title text-center">User Details</h2>
                ...
                if($num_users>0){
                    ...
                    while($rentals_row=mysqli_fetch_assoc($rentals_result))
                    {
                        $user_id=$rentals_row['user_id'];
                        $rental_start_date=$rentals_row['rental_start_date'];
                        $user_sql="SELECT name,email FROM user WHERE id=$user_id";
                        $user_result=mysqli_query($conn,$user_sql);
                        $user_row=mysqli_fetch_assoc($user_result);
                        $user_name=$user_row['name'];
                        $user_email=$user_row['email'];
                        // $rental_start_date=$user_row['rental_start_date'];
                        
                        echo '
                        <div class="try1 col-12 col-md-6 col-lg-3">
                            <div class="card overflow-hidden"> 
                                <div class="card-body">
                                    <p>User Name: '.$user_name.' </p>
                                    <p>User email: '.$user_email.' </p>
                                    <p>Rental start date: '.$rental_start_date  .' </p>
                                </div>
                            </div>
                        </div>';
                    }
                    ...
                    echo '<h4 class="never">Never been rented!</h4>;';
